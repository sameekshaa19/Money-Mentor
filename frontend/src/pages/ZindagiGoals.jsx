...
export default function ZindagiGoals() {
  ...
  const totalSIP = safeGoals.reduce((s, g) => s + g.monthlySIP, 0);
  const conflict = totalSIP > SURPLUS;
  const pct = Math.min((totalSIP / SURPLUS) * 100, 100);

  // Recalculate conflict when goals change
  const displayGoals = safeGoals.map(g => ({ ...g, conflict }));

  const removeGoal = (id) => removeZindagiGoal(id);
  ...
  const addGoal = async () => {
    setViewState('list');
    await addZindagiGoal(currentPreset, Number(customizeAmount), Number(customizeYears));
  ...
  return (
    <div className="section-page active">
      <div className="relative min-h-screen pt-10 pb-32">
        ...
        <div>
          <FinanceChatbot 
            context={{ goals, totalSIP, surplus: SURPLUS, conflict }}
            page="goals"
          />
        </div>
      </div>
